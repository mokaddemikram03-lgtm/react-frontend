  ...
  return (
    <div className="page">
      <style>{`
        ...
        .circle-icon { width:18px;height:18px;border-radius:50%;border-right:3px solid #333; }
        .level-bars { display:flex; gap:3px; }
        .level-bars .bar { width:3px;background:#333; }
        .h1 { height:17px; }
        .h2 { height:13px; }
        .h3 { height:9px; }
        ...
        table { width:100%; border-collapse:collapse; background:#d9d9d9; }
        th { text-align:left;padding:12px;font-weight:bold; }
        td { padding:14px 12px;color:#555; }
        tbody tr { border-bottom:1px solid #e5e5e5; }
        table tbody tr:nth-child(3n+1) { background:white; }
        table tbody tr:nth-child(3n+2), table tbody tr:nth-child(3n+3) { background:#f0f0f0; }
        .status { padding:8px 20px;border-radius:20px;color:white;font-weight:bold;text-transform:capitalize; }
        .status.active { background:#b3e6b3;color:#056b05; }
        .status.finished { background:#ffcccc;color:#b30000; }
            ...
            <tbody>
              {years.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.endDate}</td>
                  <td>
                    <span className={item.status === "active" ? "status active" : "status finished"}>
                      {item.status}
                    </span>
                  </td>
                  <td>
